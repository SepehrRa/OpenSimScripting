function [] = EMGChecker(FEMGdata,colname)
...
if mean(P1(sum(f<20):sum(f<100)))<2*mean(P1(sum(f<200):sum(f<500)))
    warning("poor signal to noise ratio, check sensor connectivity of %s",colname);
    g=1;
elseif mean(P1(sum(f<20):sum(f<150)))<2*mean(P1(1:sum(f<20)))
    warning('low frequency motion artifact detected in %s',colname);
    g=1;
end
...
if g==1
    figure
    subplot(2,1,1)
    plot(1:length(FEMGdata),FEMGdata)
